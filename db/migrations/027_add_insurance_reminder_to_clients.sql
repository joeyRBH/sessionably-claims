-- 027: record when a patient was reminded to finish their insurance details.
--
-- WHY
--
...
--
-- The scheduled reminder (backend/handlers/insurance_reminder.js) closes that
-- gap: 24 hours after the link went out, a patient whose insurance details are
-- still missing gets ONE email with a fresh link.
--
...
-- is enabled (disabled by default — infra/terraform/insurance-reminder.tf).
...
comment on column clients.insurance_reminder_sent_at is
  'When the automatic "finish your insurance details" email was sent to this patient. NULL means never. Set once — the scheduled reminder sends one email and stops. Not PHI: a timestamp about our own outreach.';
...
-- ---------------------------------------------------------------------------
--
--   select count(*) from clients where insurance_reminder_sent_at is not null;
--   ^^ MUST be 0 immediately after applying — nothing has sent a reminder yet.
--
--   select count(*) from information_schema.columns
--    where table_name = 'clients' and column_name = 'insurance_reminder_sent_at';
--   ^^ MUST be 1.
--
--   select indexname from pg_indexes
--    where tablename = 'clients' and indexname = 'idx_clients_awaiting_insurance_reminder';
...
-- ---------------------------------------------------------------------------
--
--   drop index if exists idx_clients_awaiting_insurance_reminder;
--   alter table clients drop column if exists insurance_reminder_sent_at;
--
...
-- EventBridge rule first (insurance_reminder_enabled = false), then drop.
